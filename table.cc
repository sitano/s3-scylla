...
table::try_flush_memtable_to_sstable(lw_shared_ptr<memtable> old, sstable_write_permit&& permit) {
  return with_scheduling_group(_config.memtable_scheduling_group, [this, old = std::move(old), permit = std::move(permit)] () mutable {
    ...
    return do_with(std::move(permit), [this, old] (sstable_write_permit& permit) {
        // TODO release permit earlier, on a sstable which had on_data_write_completed() called on its behalf

      // FIXME: indentation

      auto metadata = mutation_source_metadata{};
      const auto estimation = _compaction_strategy.adjust_partition_estimate(metadata, old->partition_count());
      auto consumer = _compaction_strategy.make_interposer_consumer(metadata, [this, old, estimation] (flat_mutation_reader reader) {
        ...
        auto monitor = std::make_unique<database_sstable_write_monitor>(sstable_write_permit::unconditional(),
            newtab, _compaction_manager, _compaction_strategy, old->get_max_timestamp());
        ...
        auto f = do_with(std::move(monitor), [newtab, &cfg, estimation, &mt = *old, reader = std::move(reader), &priority] (auto&& monitor) mutable {
            return newtab->write_components(std::move(reader), estimation, mt.schema(), cfg, mt.get_encoding_stats(), priority);
        });
      ...
      return consumer(old->make_flush_reader(old->schema(), service::get_local_memtable_flush_priority())).then([this, old] {
